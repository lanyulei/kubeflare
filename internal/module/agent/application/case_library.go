...
type caseCacheEntry = vectorCacheEntry[domain.DiagnosisCase]

// newDiagnosisCaseStore 构造案例的有界向量缓存:按"症状指纹"去重(同类故障的
// 重复案例用最新一条替换,避免冗余挤占缓存、污染 few-shot),按案例 ID 回填向量。
func newDiagnosisCaseStore(capacity int) *boundedVectorCache[domain.DiagnosisCase] {
	return newBoundedVectorCache[domain.DiagnosisCase](
		capacity,
...
func caseEmbedText(item domain.DiagnosisCase) string {
	...
	return normalizeForEmbedding(strings.Join(parts, " "))
}
