...
// Entry 是泛型有界向量缓存中的一条:领域条目 + 其 embedding 向量
...
// BoundedCache 是并发安全、有界、按"新→旧"语义维护的内存向量缓存,统一
...
type BoundedCache[T any] struct {
	mu       sync.RWMutex
	buf      []Entry[T] // 环形缓冲,容量固定为 capacity
...
// NewBoundedCache 构造有界向量缓存。capacity<=0 时回退为 1。
func NewBoundedCache[T any](capacity int, keyOf func(T) string, idOf func(T) string) *BoundedCache[T] {
	if capacity <= 0 {
		capacity = 1
	}
...
func (c *BoundedCache[T]) indexAt(pos int) int {
...
func (c *BoundedCache[T]) Add(item T, vector []float32) {
	if c == nil {
		return
	}
	c.mu.Lock()
	...
	c.prependLocked(Entry[T]{Item: item, Vector: vector, key: key})
}

// prependLocked 把 entry 设为最新元素。调用方须持有写锁。
func (c *BoundedCache[T]) prependLocked(entry Entry[T]) {
	if c.size == 0 {
		c.head = 0
	} else {
...
func (c *BoundedCache[T]) removeByKeyLocked(key string) {
	if key == "" || c.size == 0 {
		return
	}
...
func (c *BoundedCache[T]) removeAtPosLocked(pos int) {
	...
	c.buf[tailIdx] = Entry[T]{}
	c.size--
}

// UpdateVector 按 idOf 回填指定条目的向量(异步向量化完成后调用)。条目已被
// 淘汰或未启用 idOf 时静默跳过。
func (c *BoundedCache[T]) UpdateVector(id string, vector []float32) {
...
func (c *BoundedCache[T]) RemoveMatching(pred func(T) bool) int {
	if c == nil || pred == nil {
		return 0
	}
	c.mu.Lock()
...
func (c *BoundedCache[T]) Snapshot() []Entry[T] {
	if c == nil {
		return nil
	}
	c.mu.RLock()
...
func (c *BoundedCache[T]) Replace(entries []Entry[T]) {
	if c == nil {
		return
	}
	c.mu.Lock()
...
func (c *BoundedCache[T]) appendOldestLocked(entry Entry[T]) {
	if c.size == 0 {
		c.head = 0
		c.buf[0] = entry
		c.size = 1
