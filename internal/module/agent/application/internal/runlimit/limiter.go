...
// Limiter 是未注入分布式协调器时的单实例兜底限流器。生产多副本部署应通过
...
// New 构造限流器。perUser/global <=0 表示对应维度不限制。
func New(perUser int, global int) *Limiter {
	return &Limiter{
...
func (l *Limiter) Acquire(subject string) (release func(), ok bool) {
	if l == nil {
		return func() {}, true
	}
