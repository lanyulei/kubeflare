...
func TestToolSchemasAreValidJSON(t *testing.T) {
	for _, tool := range NewToolRegistry().List() {
		...
		if err := json.Unmarshal(tool.Parameters, &obj); err != nil {
			t.Errorf("tool %s schema invalid: %v", tool.ID, err)
		}
		if obj["type"] != "object" {
			t.Errorf("tool %s schema type = %v, want object", tool.ID, obj["type"])
		}
	}
}
...
func TestSanitizeAndResolveRoundTrip(t *testing.T) {
	...
	for _, tool := range registry.ToolsForAgent(domain.AGENT_TYPE_DIAGNOSTIC) {
		nameToID[SanitizeName(tool.ID)] = tool.ID
	}

	name := SanitizeName(domain.TOOL_ID_POD_GET)
	if name != "cluster_pod_get" {
		t.Fatalf("sanitizeToolName = %q, want cluster_pod_get", name)
	}
	got, ok := ResolveID(name, nameToID, registry)
	if !ok || got != domain.TOOL_ID_POD_GET {
		t.Errorf("ResolveID(%q) = %q,%v; want %q,true", name, got, ok, domain.TOOL_ID_POD_GET)
	}

	// 映射表未命中时的 '_'→'.' 兜底。
	got, ok = ResolveID("cluster_node_list", map[string]string{}, registry)
	if !ok || got != domain.TOOL_ID_NODE_LIST {
		t.Errorf("fallback resolveToolID = %q,%v; want %q,true", got, ok, domain.TOOL_ID_NODE_LIST)
	}

	// 未知工具。
	if _, ok := ResolveID("totally_unknown", map[string]string{}, registry); ok {
		t.Error("resolveToolID should fail for unknown tool")
	}
}

// TestResolveScopeOverrides 验证 LLM 参数覆盖预设 scope 的语义(解析已下沉至
// domain.ResolveScope)。
func TestResolveScopeOverrides(t *testing.T) {
	...
	// 非法 JSON 返回错误。
	if _, err := domain.ResolveScope(base, `{bad`); err == nil {
		t.Error("ResolveScope should error on invalid JSON")
	}
}

// TestValidateAgainstSchema 验证轻量参数校验。
func TestValidateAgainstSchema(t *testing.T) {
	schema := json.RawMessage(schemaPodGet) // required: resource_name
...
func TestValidateAgainstSchemaEnum(t *testing.T) {
	schema := json.RawMessage(schemaWorkloadGet) // resource_kind enum: deployment/statefulset/daemonset
