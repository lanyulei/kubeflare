...
func (s *Service) validateInvocation(
	...
	toolID, ok := agenttool.ResolveID(inv.Name, nameToID, s.toolRegistry)
	if !ok {
		return plannedToolCall{}, validationResult{reason: fmt.Sprintf("未知工具 %q", inv.Name)}
	}
	tool, ok := s.toolRegistry.Get(toolID)
	if !ok || !tool.Enabled || !tool.ReadOnly || !agenttool.AllowedForAgent(tool, agent.Type) {
	...
	if err := agenttool.ValidateAgainstSchema(tool.Parameters, inv.Arguments); err != nil {
...
func (s *Service) buildToolSpecs(tools []domain.ToolDefinition) ([]aiapplication.ToolSpec, map[string]string) {
	...
	for _, tool := range tools {
		name := agenttool.SanitizeName(tool.ID)
