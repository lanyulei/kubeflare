...
type feedbackCacheEntry = appvector.Entry[domain.RouteFeedback]

// newRouteFeedbackStore 构造路由样例的有界向量缓存:按"消息指纹"去重(同一问法的
// 重复确认用最新一条替换),按反馈 ID 回填向量。
func newRouteFeedbackStore(capacity int) *appvector.BoundedCache[domain.RouteFeedback] {
	return appvector.NewBoundedCache[domain.RouteFeedback](
		capacity,
...
func (s *Service) warmupFeedbackVectors(ctx context.Context, cached []feedbackCacheEntry) {
	...
	for index := range cached {
		texts[index] = appvector.NormalizeForEmbedding(cached[index].Item.Message)
	}
	offset := 0
...
func (s *Service) similarRouteFeedback(ctx context.Context, message string, limit int) []domain.RouteFeedback {
	...
	if strings.TrimSpace(message) != "" {
		if vector := s.embedQuery(ctx, message); len(vector) > 0 {
			...
			matched := appvector.TopKByVector(snapshot, vector, getVec, limit, appvector.MIN_SEMANTIC_SCORE)
			if len(matched) > 0 {
				return feedbackCacheEntryToItems(matched)
			}
			// 语义无命中且候选向量与 query 维度不一致(换模型/版本不一致)时告警。
			if appvector.DimensionMismatch(vector, snapshot, getVec) {
