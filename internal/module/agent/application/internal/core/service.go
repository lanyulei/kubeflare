...
import (
	...
	appcache "github.com/lanyulei/kubeflare/internal/module/agent/application/internal/cache"
	apprunlimit "github.com/lanyulei/kubeflare/internal/module/agent/application/internal/runlimit"
...
type Options struct {
	...
	ToolExecutors []agenttool.SourceToolExecutor
...
type Service struct {
	...
	feedbackStore *appvector.BoundedCache[domain.RouteFeedback]
	// caseRepo / caseStore 是诊断案例库的可选仓储(类型断言获取,缺失即关闭)与
	// 内存案例缓存(注入热路径只读内存,不查库)。
	caseRepo  domain.DiagnosisCaseRepository
	caseStore *appvector.BoundedCache[domain.DiagnosisCase]
	...
	runLimiter               *apprunlimit.Limiter
	semaphore                sharedcoord.Semaphore
	eventBus                 sharedcoord.EventBus
	instanceID               string
	mcpStatusProvider        func() []RuntimeMCPServerStatus
	...
	suppressedCaseRuns *appcache.BoundedStringSet
	...
	toolProviders agenttool.ProviderSet
	reloadMu      sync.Mutex
}
...
func NewService(options Options) *Service {
	...
	toolRegistry := agenttool.NewToolRegistry()
	...
	if len(options.ToolExecutors) > 0 {
		toolExecutor = agenttool.NewToolDispatcher(toolRegistry, options.ToolExecutors...)
	}
	...
	service := &Service{
		...
		runLimiter:               apprunlimit.New(options.Loop.MaxConcurrentRunsPerUser, options.Loop.MaxConcurrentRuns),
		...
		suppressedCaseRuns:       appcache.NewBoundedStringSet(SUPPRESSED_CASE_RUNS_CAPACITY),
...
func (s *Service) StreamRun(ctx context.Context, userID string, agentType string, req RunAgentRequest) (<-chan domain.AgentRunEvent, error) {
	...
	req.SetRoutedSkillID(route.SkillID)
