...
func newLoopTestService(gen aiapplication.AssistantGenerator, executor agenttool.ToolExecutor) *Service {
...
func TestRunLoopHappyPath(t *testing.T) {
	podGet := agenttool.SanitizeName(domain.TOOL_ID_POD_GET)
	...
	go func() {
		defer close(events)
		answer, alive, err = s.runLoop(context.Background(), context.Background(), events, run, diagnosticAgent(s), RunAgentRequest{Message: "why pod failing", ClusterID: "c1"}, nil, "", &runStats{})
	}()
	evts := drain(events)
...
func TestRunLoopRejectsUnknownTool(t *testing.T) {
	...
	go func() {
		defer close(events)
		_, _, _ = s.runLoop(context.Background(), context.Background(), events, domain.AgentRun{ID: "r"}, diagnosticAgent(s), RunAgentRequest{Message: "x", ClusterID: "c"}, nil, "", &runStats{})
	}()
	drain(events)
...
func TestRunLoopMaxStepsForceConclude(t *testing.T) {
	podList := agenttool.SanitizeName(domain.TOOL_ID_POD_LIST)
	...
	go func() {
		defer close(events)
		answer, _, err = s.runLoop(context.Background(), context.Background(), events, domain.AgentRun{ID: "r"}, diagnosticAgent(s), RunAgentRequest{Message: "x", ClusterID: "c"}, nil, "", &runStats{})
	}()
	drain(events)
...
func TestRunLoopGeneratorError(t *testing.T) {
	...
	go func() {
		defer close(events)
		_, alive, err = s.runLoop(context.Background(), context.Background(), events, domain.AgentRun{ID: "r"}, diagnosticAgent(s), RunAgentRequest{Message: "x", ClusterID: "c"}, nil, "", &runStats{})
	}()
	drain(events)
...
func TestRunLoopCarriesChatHistory(t *testing.T) {
	...
	go func() {
		defer close(done)
		_, _, _ = s.runLoop(context.Background(), context.Background(), events, domain.AgentRun{ID: "r"}, diagnosticAgent(s), RunAgentRequest{Message: "那怎么修复", ClusterID: "c"}, chatHistory, "", &runStats{})
		close(events)
	}()
	drain(events)
	<-done
