...
// useful_pct,真正衡量智能改进对诊断质量的影响。一次 run 仅允许保留一条反馈。
...
// 反馈收集,否则静默关闭。CreateRunFeedback 按 RunID 只创建一次,重复反馈返回冲突。
type RunFeedbackRepository interface {
	CreateRunFeedback(ctx context.Context, feedback RunFeedback) (RunFeedback, error)
	ListRunFeedbackByRunIDs(ctx context.Context, runIDs []string) (map[string]RunFeedback, error)
